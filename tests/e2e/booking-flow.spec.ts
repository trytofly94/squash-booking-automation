...
test.describe('Squash Booking E2E Flow', () => {
  ...
  test('should navigate to booking page successfully', async () => {
    ...
    await test.step('Check for calendar or booking interface', async () => {
      ...
      const hasCalendar =
        (await page.locator('.calendar, [class*="calendar"], .booking-calendar').count()) > 0;
      const hasTimeSlots =
        (await page.locator('.time-slot, [class*="slot"], .available').count()) > 0;
      const hasDatePicker =
        (await page.locator('input[type="date"], .date-picker, [class*="date"]').count()) > 0;
      ...
      const bookingElements = await page.evaluate(() => {
        const findElements = (selectors: string[]) => {
          return selectors.map(selector => ({
            ...
            visible: Array.from(document.querySelectorAll(selector)).filter(
