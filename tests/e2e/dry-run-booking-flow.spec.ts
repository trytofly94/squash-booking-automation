...
test.describe('Squash Booking Automation - Dry Run End-to-End', () => {
  ...
  test('should validate website accessibility and structure', async ({ page }) => {
    ...
    const hasCalendarElements =
      (await page.locator('table, .calendar, .booking, [data-date]').count()) > 0;
    const hasTimeElements =
      (await page.locator('[data-time], [data-start], .time, .slot').count()) > 0;
    const hasInteractiveElements =
      (await page.locator('button, input, select, .clickable').count()) > 0;
    ...
    const elementCounts = await page.evaluate(() => {
      return {
        ...
        dataAttributes: document.querySelectorAll(
          '[data-date], [data-time], [data-start], [data-court]'
        ).length,
      };
    });
  ...
  test('should validate selector strategies against current website', async ({ page }) => {
    ...
    const selectors = {
      // Calendar elements
      calendar:
        '#booking-calendar-container, .calendar, [data-testid="calendar"], .booking-calendar',
  ...
  test('should analyze slot availability patterns', async ({ page }) => {
    ...
    const slotAnalysis = await page.evaluate(() => {
      ...
      const timeElements = document.querySelectorAll(
        '[class*="time"], [class*="slot"], [id*="time"], [id*="slot"]'
      );
